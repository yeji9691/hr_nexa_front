...
if (!nexacro.VirtualFile) {
	...
	_pVirtualFile.read = function (nLength, strCharset) {
		...
		if (isNaN(nLength) == false) {
			if (nexacro._Browser == "Runtime" && nexacro._SystemType.toLowerCase() == "win64") {
				var _intVal = nexacro._toInt(nLength);
				var _numVal = Number(nLength);
				if (isNaN(_numVal) == false && _numVal > _intVal && _intVal < 0) {
					nLength = _numVal;
				}
				else {
					nLength = _intVal;
