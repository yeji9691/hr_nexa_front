...
if (!nexacro.DataObject) {
	...
	nexacro.DataObjectErrorEventInfo = function (obj, id, errortype, errormsg, errorobj, statuscode, requesturi, locationuri) {
	...
	nexacro.DataObjectEventInfo = function (obj, id, response, svcid, url, reason) {
		this.id = this.eventid = id || "onload";
		this.fromobject = this.fromreferenceobject = obj;
	...
	_pDataObject.request = function (svcid, method, url, jsonparam) {
		...
		url = nexacro._getServiceLocation(url, baseurl);
		if (url.length && this.parent) {
			...
			if (loadmanager) {
				...
				loadmanager.loadDataObjectModule(url, svcid, method, this, jsonparam, async, undefined, false, service);
			}
		}
		else {
			this._endLoad(-1, "empty url", 1);
	...
	_pDataObject.on_fire_onsuccess = function (response, svcid, url, reason) {
		var event = this.onsuccess;
		if (event && event._has_handlers) {
			var evt = new nexacro.DataObjectEventInfo(this, "onsuccess", response, svcid, url, reason);
			event._fireEvent(this, evt);
		}
	};

	_pDataObject.on_fire_onerror = function (errortype, errormsg, errorobj, statuscode, requesturi, locationuri) {
		if (this.onerror && this.onerror._has_handlers) {
			var evt = new nexacro.DataObjectErrorEventInfo(this, "onerror", errortype, errormsg, errorobj, statuscode, requesturi, locationuri);
	...
	_pDataObject._on_success = function (response, svcid, url, jsonobj, method) {
		var pThis = this;
		var ret = this.on_fire_onsuccess(response, svcid, url, 2);
	...
	_pDataObject._onDataObjectRequestError = function (obj, bfireevent, errorobj, errortype, url, returncode, requesturi, locationuri, extramsg, svcid) {
		var ret = false;
	...
	_pRESTAPIItem.on_load_dataobject = function (data, cookie, last_modified) {
		...
		if (bcache) {
			...
			if (!d_cache) {
				nexacro._DataObjectCacheList[this.path] = new nexacro._DataObjectCache(dataobject, last_modified, this.version, jsonobj);
			}
			else {
				d_cache.dataobject = dataobject;
				d_cache.last_modified = last_modified ? last_modified : "";
				d_cache.version = this.version;
				d_cache.data = jsonobj;
			}
		}
	...
	_pRESTAPIItem.on_error = function (errstatus, fireerrorcode, returncode, locationurl, extramsg) {
		...
		if (n > 0) {
			for (var i = 0; i < n; i++) {
				...
				var target = item.target;
				if (target._is_alive != false) {
					ret = item.callback.call(target, this.svcid, -1, "FAILED", fireerrorcode, returncode, this.path, locationurl, extramsg, dataobj);
					if (ret) {
						if (this._usewaitcursor) {
